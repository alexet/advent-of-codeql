...
for year in os.listdir('everybody_codes'):
    if os.path.isdir(os.path.join('everybody_codes', year)):
        # check that it is not a hidden directory
        if year.startswith('.'):
            continue
        print(year)
        for day in os.listdir(os.path.join('everybody_codes', year)):
            dayDir = os.path.join('everybody_codes', year, day)
            if os.path.isdir(dayDir):
                print(dayDir)
                for name in ["real.in", "test.in"]:
                    file = os.path.join(dayDir, name)
                                ...
                                "extensions":[
                                    {
                                        ...
                                        {
                                            'pack': "alexet/advent-of-code",
                                            'extensible' : baseName+"_everybody_codes"
                                        },
                                        "data":
                                        [[year, day, data]]
                                    }
